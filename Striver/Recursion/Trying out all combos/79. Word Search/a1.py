...
class Solution:
    ...
    def backTrack(self, row, col, m, n , visited, curChar, board, word):
        # basecase
        if row < 0 or row >= m or col < 0 or col >= n or curChar >= len(word) or visited[row][col] == True or board[row][col] != word[curChar]:
            return False
        ...
        return (up or down or left or right) or (curChar + 1 == len(word) and word[curChar] == word[-1])
