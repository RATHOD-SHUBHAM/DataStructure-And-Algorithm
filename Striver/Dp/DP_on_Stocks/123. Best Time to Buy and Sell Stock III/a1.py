...
class Solution:
    ...
    def recursion(self, idx, buy, k, prices, n):
        ...
        if buy == True:
            ...
            no_purchase = self.recursion(idx+1, True, k, prices, n) - 0
            ...
            sell = prices[idx] + self.recursion(idx+1, True, k - 1, prices, n) # prices[idx]== sell, so equation will be like sell - buy + (future profit)
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        dp = [[[0 for _ in range(count+1)] for _ in range(2)]for _ in range(n+1)]
            ...
            for buy in range(2):
                for k in range(1, count+1):
                    if buy == 0:  # Can buy
                        ...
                        sell = prices[idx] + dp[idx+1][0][k-1] # prices[idx]== sell, so equation will be like sell - buy + (future profit)
