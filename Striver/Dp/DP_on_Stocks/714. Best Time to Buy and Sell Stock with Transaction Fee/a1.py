class Solution:
    def maxProfit(self, prices: List[int], fee: int) -> int:
        ...
        return self.recursion(idx, buy, prices, fee, n)
    
    def recursion(self, idx, buy, prices, fee, n):
        if idx == n:
        ...
        if buy == True:
            ...
            purchase = self.recursion(idx+1, False, prices, fee, n) - prices[idx] # prices[idx]== buy, because this is the stock i buy

            # The bool value is True stating I have not purchased any stock, I am allowed to buy stock in future
            no_purchase = self.recursion(idx+1, True, prices, fee, n) - 0
            ...
            sell = prices[idx] + self.recursion(idx+1, True, prices, fee, n) # prices[idx]== sell, so equation will be like sell - buy + (future profit)
            sell -= fee

            # if i decide not to sell, I have a stock purchased previously,
            # I cannot purchase any new one unless I sell the previous one
            no_sell = 0 + self.recursion(idx+1, False, prices, fee, n)
...
class Solution:
    def maxProfit(self, prices: List[int], fee: int) -> int:
        ...
        return self.recursion(idx, buy, memo, prices, fee, n)
    
    def recursion(self, idx, buy, memo, prices, fee, n):
        if idx == n:
            # if i would have bought a stock before and have not sold it
            # or there is no stock
            # Either way my profit will be 0
        ...
        if buy == True:
            ...
            # Change the bool value to buy stating we have a bought a stock
            purchase = self.recursion(idx+1, False, memo, prices, fee, n) - prices[idx] # prices[idx]== buy, because this is the stock i buy

            # The bool value is True stating I have not purchased any stock, I am allowed to buy stock in future
            no_purchase = self.recursion(idx+1, True, memo, prices, fee, n) - 0
            ...
            # if i decide to sell, i can change the bool value to true, stating I am allowed to buy stock in future
            sell = prices[idx] + self.recursion(idx+1, True, memo, prices, fee, n) # prices[idx]== sell, so equation will be like sell - buy + (future profit)
            sell -= fee

            # if i decide not to sell, I have a stock purchased previously,
            # I cannot purchase any new one unless I sell the previous one
            no_sell = 0 + self.recursion(idx+1, False, memo, prices, fee, n)
...
# ------------------------------ Space Optimization Approach with Single Loop ------------------------------
class Solution:
    def maxProfit(self, prices: List[int], fee: int) -> int:
        n = len(prices)
